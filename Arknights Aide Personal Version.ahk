...
$^!H::																							;; ctrl+alt+H 顯示操作列表10秒
	...
	if (!Canceled) {																			;; 正常施放
		ToolTip, 1.   ctrl+alt+H`t: Display the operation list 10 seconds`n2.   ctrl+alt+D`t: Turn developer mode on / off`n3.   ctrl+alt+T`t: Suspend / Restart all hotkeys except itself`n4.   ctrl+alt+R`t: Reload program`n5.   ctrl+alt+L`t: Exit program`nThe following operations can only be performed in developer mode : `n6.   ctrl+alt+M`t: Move the mouse to the specified position`n7.   ctrl+alt+G`t: Get the current values of each variable`n8.   ctrl+alt+C`t: Query the color code of the specified position`nThe following operations can only be performed on the simulator : `n9.   P`t`t: Pause the game at beginning`n10. K`t`t: Pause the game after it goes about 1 frame`n11. H`t`t: Click the operator or props by current mouse position while game paused`n12. M`t`t: Deploy the operator or props by current mouse position while game paused
